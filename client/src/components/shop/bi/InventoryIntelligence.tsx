...
const InventoryIntelligence: React.FC = () => {
    ...
    const stats = [
        { label: 'Categories Tracked', value: totalCategories > 0 ? `${totalCategories}` : '--', icon: ICONS.dollarSign, color: 'text-indigo-600', bg: 'bg-indigo-50' },
        { label: 'Total Units Sold', value: totalUnits > 0 ? totalUnits.toLocaleString() : '--', icon: ICONS.history, color: 'text-emerald-600', bg: 'bg-emerald-50' },
        { label: 'Top Category', value: topCategory ? topCategory.category : '--', icon: ICONS.barChart, color: 'text-amber-600', bg: 'bg-amber-50' },
    ];
                        ...
                        <div className="space-y-6">
                            {topCategory && (
                                <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100 relative overflow-hidden group">
                                    ...
                                    <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Top Seller</p>
                                    <p className="text-sm font-black text-emerald-900 mt-1">{topCategory.category}</p>
                                    <p className="text-[10px] text-emerald-700 mt-1">{Number(topCategory.unitsSold).toLocaleString()} units sold</p>
                                </div>
                            )}
                            {slowCategory && slowCategory.category !== topCategory?.category && (
                                <div className="p-4 bg-rose-50 rounded-2xl border border-rose-100 relative overflow-hidden group">
                                    ...
                                    <p className="text-[10px] font-black text-rose-600 uppercase tracking-widest">Slowest Category</p>
                                    <p className="text-sm font-black text-rose-900 mt-1">{slowCategory.category}</p>
                                    <p className="text-[10px] text-rose-700 mt-1">{Number(slowCategory.unitsSold).toLocaleString()} units sold</p>
                                </div>
                            )}
                        </div>
                    ) : (
