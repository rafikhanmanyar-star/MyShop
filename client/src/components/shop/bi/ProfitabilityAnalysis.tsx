        ...
        <div className="space-y-8 animate-in slide-in-from-bottom duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <Card className="border-none shadow-sm dark:shadow-none dark:bg-slate-900/90 dark:border dark:border-slate-600 p-8 bg-card space-y-8">
                    <h4 className="font-bold text-foreground dark:text-slate-200 uppercase tracking-widest text-xs">Revenue Breakdown</h4>
                    {hasData ? (
                        <div className="space-y-6 pt-4">
                            {[
                                ...
                                <div key={i} className="flex items-center gap-6">
                                    <div className="w-32 text-xs font-semibold text-muted-foreground uppercase tracking-widest text-right">{step.label}</div>
                                    ...
                                    <div className="w-24 text-sm font-semibold text-foreground font-mono text-right">{step.val}</div>
                                </div>
                            ))}
                        </div>
                    ) : (
                        <div className="flex items-center justify-center py-16">
                            <p className="text-xs text-muted-foreground">No revenue data available yet. Complete sales to see the breakdown.</p>
                        </div>
                    )}
                </Card>
                ...
                <Card className="border-none shadow-sm dark:shadow-none dark:bg-slate-900/90 dark:border dark:border-slate-600 p-8 bg-card flex flex-col h-full">
                    <h4 className="font-bold text-foreground dark:text-slate-200 uppercase tracking-widest text-xs mb-8">Top Categories by Revenue</h4>
                        ...
                        {topCategories.length > 0 ? topCategories.map((cat: any, i: number) => (
                            <div key={i} className="flex items-center justify-between p-4 border border-slate-50 dark:border-slate-700 rounded-2xl hover:border-indigo-100 dark:hover:border-indigo-500/40 transition-all cursor-pointer group">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 bg-muted/80 dark:bg-slate-800 rounded-xl flex items-center justify-center text-slate-300 dark:text-slate-600 font-semibold text-xl group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                        {cat.category.charAt(0)}
                                    </div>
                                    <div>
                                        <p className="text-sm font-semibold text-foreground tracking-tight">{cat.category}</p>
                                        <p className="text-xs text-muted-foreground font-bold uppercase mt-0.5">{Number(cat.unitsSold).toLocaleString()} units sold</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 font-mono leading-none">{CURRENCY} {Number(cat.revenue).toLocaleString()}</p>
                                </div>
                            </div>
                        )) : (
                            <div className="flex items-center justify-center py-16">
