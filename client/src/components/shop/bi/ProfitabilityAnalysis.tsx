                        ...
                        <div className="space-y-6 pt-4">
                            {[
                                ...
                                <div key={i} className="flex items-center gap-6">
                                    ...
                                    <div className="w-24 text-sm font-black text-foreground font-mono text-right">{step.val}</div>
                                </div>
                            ))}
                        </div>
                    ) : (
                        ...
                        {topCategories.length > 0 ? topCategories.map((cat: any, i: number) => (
                            <div key={i} className="flex items-center justify-between p-4 border border-slate-50 dark:border-slate-700 rounded-2xl hover:border-indigo-100 dark:hover:border-indigo-500/40 transition-all cursor-pointer group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-sm font-black text-foreground tracking-tight">{cat.category}</p>
                                        <p className="text-[10px] text-muted-foreground font-bold uppercase mt-0.5">{Number(cat.unitsSold).toLocaleString()} units sold</p>
                                    </div>
                                </div>
