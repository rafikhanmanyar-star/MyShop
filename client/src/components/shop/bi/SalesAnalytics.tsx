        ...
        <div className="space-y-8 animate-in slide-in-from-right duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="border-none shadow-sm dark:shadow-none dark:bg-slate-900/90 dark:border dark:border-slate-600 p-6 bg-card space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">POS Sales</p>
                            <p className="text-2xl font-black text-foreground font-mono">{CURRENCY} {posRevenue.toLocaleString()}</p>
                        </div>
                    </div>
                ...
                <Card className="border-none shadow-sm dark:shadow-none dark:bg-slate-900/90 dark:border dark:border-slate-600 p-6 bg-card space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Mobile App Orders</p>
                            <p className="text-2xl font-black text-foreground font-mono">{CURRENCY} {mobileRevenue.toLocaleString()}</p>
                        </div>
                    </div>
                    <div className="flex justify-between text-xs">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <Card className="border-none shadow-sm dark:shadow-none dark:bg-slate-900/90 dark:border dark:border-slate-600 p-8 bg-card flex flex-col gap-8">
                    <h4 className="font-bold text-foreground dark:text-slate-200 uppercase tracking-widest text-[10px]">Revenue by Category Mix</h4>
                            ...
                            return (
                                <div key={i} className="space-y-3 cursor-pointer group">
                                    <div className="flex justify-between items-end">
                                        <div>
                                            <p className="text-sm font-black text-foreground group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{cat.category}</p>
                                            <p className="text-[10px] text-muted-foreground font-bold mt-0.5">{cat.unitsSold} units sold</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-sm font-black text-foreground font-mono tracking-tight">
                                                {CURRENCY} {Number(cat.revenue).toLocaleString()}
                                            </p>
                                            <p className="text-[10px] text-indigo-500 dark:text-indigo-400 font-black tracking-widest uppercase">{percent.toFixed(1)}% Share</p>
                                        </div>
                                    </div>
                ...
                <Card className="border-none shadow-sm dark:shadow-none dark:bg-slate-900/90 dark:border dark:border-slate-600 p-8 bg-card space-y-8">
                    <div className="flex justify-between items-center">
                        <h4 className="font-bold text-foreground dark:text-slate-200 uppercase tracking-widest text-[10px]">Daily Revenue Trend</h4>
                        <div className="flex gap-3">
                            <span className="flex items-center gap-1 text-[9px] font-bold text-indigo-600 dark:text-indigo-400">
                        ...
                        {salesTrend.slice(-14).map((data: any, idx: number) => {
                            ...
                            return (
                                <div key={idx} className="flex-1 flex flex-col items-center gap-0 relative group cursor-pointer h-full justify-end">
                                    ...
                                    <span className="text-[7px] font-black text-muted-foreground uppercase tracking-tighter mt-1 whitespace-nowrap">{data.timestamp}</span>
                                </div>
                            );
                        })}
                    </div>
            ...
            <Card className="border-none shadow-sm dark:shadow-none dark:bg-slate-900/90 dark:border dark:border-slate-600 overflow-hidden bg-card">
                <div className="p-6 border-b border-slate-50 dark:border-slate-700 bg-muted/80/50 dark:bg-slate-800/80 flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                        <span className="px-2 py-1 bg-indigo-100 text-indigo-600 dark:bg-indigo-950/60 dark:text-indigo-400 text-[9px] font-black rounded uppercase">POS</span>
                        <span className="px-2 py-1 bg-emerald-100 text-emerald-600 dark:bg-emerald-950/60 dark:text-emerald-400 text-[9px] font-black rounded uppercase">Mobile</span>
                    </div>
                </div>
                        ...
                        <tbody className="divide-y divide-slate-50 dark:divide-slate-700/80 text-xs">
                            {recentTransactions.length > 0 ? recentTransactions.slice(0, 10).map((tx: any, i: number) => (
                                <tr key={i} className="hover:bg-muted/50 dark:hover:bg-slate-800/50 transition-colors font-medium">
                                    ...
                                    <td className="px-6 py-4 text-right font-black text-foreground">{Number(tx.amount || 0).toLocaleString()}</td>
