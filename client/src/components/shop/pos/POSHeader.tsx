...
const POSHeader: React.FC = () => {
    ...
    return (
        <div className="bg-white border-b border-slate-200 z-30 sticky top-0 shadow-sm flex-shrink-0">
                ...
                <div className="flex items-center gap-4 md:gap-6 min-w-0 flex-1">
                    <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
                        ...
                        <div className="min-w-0">
                            <h1 className="text-base md:text-lg font-bold text-slate-900 tracking-tight leading-none uppercase truncate">MyShop <span className="text-blue-600">POS</span></h1>
                            <div className="flex items-center gap-2 mt-0.5 md:mt-1">
                                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 flex-shrink-0"></span>
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider truncate">Terminal Online</span>
                            </div>
                        </div>
                    </div>

                    <div className="h-6 md:h-8 w-px bg-slate-200 hidden lg:block flex-shrink-0"></div>
                    ...
                    {currentShift ? (
                        <div className="hidden lg:flex items-center gap-3 xl:gap-6 min-w-0">
                            <div className="flex flex-col min-w-0">
                                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5 pl-0.5">Location</span>
                                <div className="min-w-0 max-w-[140px] px-2 md:px-3 py-1 md:py-1.5 rounded-lg bg-slate-50 border border-slate-200">
                                    <span className="text-xs font-semibold text-slate-700 truncate block">{locationName ?? '—'}</span>
                                </div>
                            </div>
                            <div className="flex flex-col min-w-0">
                                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5 pl-0.5">Station</span>
                                <div className="min-w-0 max-w-[120px] px-2 md:px-3 py-1 md:py-1.5 rounded-lg bg-slate-50 border border-slate-200">
                                    <span className="text-xs font-semibold text-slate-700 truncate block">{stationName ?? '—'}</span>
                                </div>
                            </div>
                        </div>
                    ) : (
            ...
            <div className="flex items-center gap-3 md:gap-6 flex-shrink-0 min-w-0">
                <div className="hidden lg:flex flex-col items-end flex-shrink-0">
                    <div className="text-xs md:text-sm font-bold text-slate-900 tabular-nums">
                ...
                <div className="flex items-center gap-2 md:gap-4 pl-4 md:pl-6 border-l border-slate-200 min-w-0">
                    <div className="flex flex-col items-end min-w-0">
                        <span className="text-xs md:text-sm font-bold text-slate-900 leading-tight truncate max-w-[100px] md:max-w-[120px]">{user?.name || 'Cashier Admin'}</span>
                        <span className="text-[10px] font-bold text-blue-600 uppercase tracking-wider mt-0.5 truncate">
                            {user?.role?.replace('_', ' ') || 'Manager'}
                        </span>
                    </div>
                    <div className="relative flex-shrink-0">
                        <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center overflow-hidden">
