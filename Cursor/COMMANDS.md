...
| Command | Description |
|--------|-------------|
| `npm run dev` | Starts the local API (**http://localhost:3001**), **POS web client** (**http://localhost:5173**), and **marketing website** (**http://localhost:5190**). No Electron, mobile, or rider. |
...
**What it does:** These commands only run dev servers (no installable build, no commit, no push to GitHub). **`npm run dev`** starts the local API, POS web client (**5173**), and marketing website (**5190**). **`npm run electron:dev`** starts the local API, mobile and rider Vite apps, and Electron. **`npm run dev:pos:cloud`** starts only the POS web app against your configured cloud `VITE_API_URL`.
...
|--------|-------------------|----------------------------|
| **Local test** | `npm run dev` (API + POS + website), `npm run electron:dev` (API + mobile + rider + Electron) | — (run-only; use installable release when ready to ship) |
