...
export class DailyReportService {
  async getSummary(tenantId: string, dateStr: string, branchId: string | null): Promise<DailyReportSummary> {
    ...
    const netProfitDaily = await this.computeNetProfitMargin(tenantId, start, end, branchId);
  ...
  /** Line margin for `[start, end)` — shared by daily summary and range profit totals. */
  private async computeNetProfitMargin(
    tenantId: string,
    start: string,
    end: string,
    branchId: string | null
    ...
    const db = getDatabaseService();
    const branchFilter = branchId ? 'AND s.branch_id = $4' : '';
    const mobileBranchFilter = branchId ? 'AND mo.branch_id = $4' : '';
    const returnBranchFilter = branchId ? 'AND r.branch_id = $4' : '';
    const params = branchId ? [tenantId, start, end, branchId] : [tenantId, start, end];
  ...
  /** Net line margin over a UTC `[start, end)` window (for multi-month dashboard profit). */
  async getProfitForRange(
    tenantId: string,
    startIso: string,
    endIso: string,
    branchId: string | null
    ...
    const totalProfit = await this.computeNetProfitMargin(tenantId, startIso, endIso, branchId);
    const days = Math.max(1, Math.ceil((endMs - startMs) / 86400000));
    return { days, totalProfit, avgProfitPerDay: totalProfit / days };
  }
